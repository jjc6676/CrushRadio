// Crush Radio — Cloudflare Worker
// Routes:
//   /             → coming-soon page (HOME_HTML, inlined at build time from index.html)
//   /code         → live feed of GitHub commits + PRs + issues (server-rendered)
//   /robots.txt   → robots
...
export default {
  async fetch(request) {
    ...
    return new Response("Not Found — try / or /code", {
      status: 404,
      headers: { "content-type": "text/plain; charset=utf-8" },
    });
  },
...
async function renderCodePage() {
  ...
  // GitHub's /issues endpoint includes PRs; filter to real issues only.
